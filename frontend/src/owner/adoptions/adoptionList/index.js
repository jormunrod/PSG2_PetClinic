...
export default function OwnerAdoptionList() {
  ...
  async function fetchData() {
    const petsInAdoptionResponse = await fetchWithPricingInterceptor(
      "/api/v1/pets/adoptions/available",
      {
        headers: {
    ...
    const adoptionRequestsPromises = petsInAdoptionData.map((pet) =>
      fetchAdoptionRequestsByPetId(pet.id)
    );
  ...
  function triedToAdoptEarlier(petId) {
    ...
    try {
      adoptionRequestsForPet = adoptionRequests.find((requests) =>
        requests.some((request) => request.pet.id === petId)
      );
      if (!!adoptionRequestsForPet) {
        adoptionRequestId = adoptionRequestsForPet.find(
          (request) => request.applicant.user.id === userId
        ).id;
        ...
        <div style={{ marginLeft: "20%", marginRight: "20%", marginTop: "5%" }}>
          <Container style={{ marginTop: "15px" }} fluid>
            ...
            <Table className="mt-4">
              ...
              <tbody>{getPetsInAdoptionList()}</tbody>
            </Table>
          </Container>
        </div>
      </On>
